...
class TopPlayerRatingStrategy(AnalysisStrategy):
    ...
    def analyze(self, *args):
        ...
        top_players = \
            player_attr_df.groupby('player_api_id').mean().sort_values('overall_rating', ascending=False).head(
                players_num)[
                'overall_rating']
        # Merge the player ids with the player table to get the names of the top players
        top_players = players_df.merge(top_players, left_on='player_api_id', right_index=True)[
